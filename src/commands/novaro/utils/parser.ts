...
export const parseCharGenConfig = (
	...
	if (!config) {
		...
		return [
			randomNumGenerator(0, firstArgMax),
			randomNumGenerator(0, secondArgMax),
		];
	}

	// attempt to split the config into a firstArg and a secondArg integer
	const [firstArg, secondArg] = config.split("/").map((x) => parseInt(x));

	// returns the firstArg and secondArg
	// if NaN, choose a random number instead
	return [
		isNaN(firstArg) || notInRange(firstArg, firstArgMax)
			? randomNumGenerator(0, firstArgMax)
			: firstArg,
		isNaN(secondArg) || notInRange(secondArg, secondArgMax)
			? randomNumGenerator(0, secondArgMax)
			: secondArg,
	];
};
